			...
			<div class="content-wrapper">
				<section class="content-header">
				  <div class="container-fluid">
					...
					<div class="row mb-2">
					  <div class="col-sm-12">
						<h4 style="text-align:center;"><b>Lead Reports</b></h4>
					  </div>
					</div>
				  </div><!-- /.container-fluid -->
				...
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
					...
					<div class="row">
						...
						<select class="form-control" name="chapter" id="chapter">
							...
							$get_chapter = "SELECT * FROM chapters WHERE id='$chapter'";
							$res_chapter = mysqli_query($conn,$get_chapter);
							$row_chapter=mysqli_fetch_array($res_chapter);
							?>
							<option value="<?php echo $chapter; ?>"><?php echo $row_chapter['chapter_name']; ?></option>
							<?php }?>
							<option value=''>Select Chapter</option>
							<?php
							include("config.php");
							$get_chapter = "SELECT * FROM `chapters`";
							$res_chapter = mysqli_query($conn,$get_chapter);
							while($row_chapter=mysqli_fetch_array($res_chapter))
							{
								echo "<option value=".$row_chapter['id'].">".$row_chapter['chapter_name']."</option>";
							} 
							?>
						</select>
						</div>
					  ...
					  <table id="example2" class="table table-bordered table-striped">
						...
						<tbody >
							...
							if(isset($_POST['search']))
							{
								...
								$sql_leads = "select * from leads where lead_type='$lead_type' and country_id='$country_id' and chapter_id='$chapter_id'";
								...
								while($row_leads=mysqli_fetch_array($res_leads))
								{
									...
									echo $product_id = $row_leads['product_id'];
								...
								?>
								<tr>
								    <td colspan='9' align='center'> No Leads For This Country </td>  
